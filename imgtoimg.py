...
class PicMerge:
    ...
    def __init__(self, pic_path, mode='RGB', pic_folder='G:/手机视频/图片/图片/Wallpapers'):
    ...
    def merge(self):
        ...
        picture.show()
        picture.save('result2.jpg')
        print('Work Done...')

    def structure_similarity(self, section, candidate):
        section = Image.fromarray(section).convert('L')
        one_hot = self.pic_code(np.array(section.resize((200, 200), Image.ANTIALIAS)))
        ...
        for idx, pic in tqdm(enumerate(pic_list), desc='CODE'):
            if pic.split('.')[-1] in suffix:
                ...
                try:
                    ...
                    results[idx] = self.pic_code(np.array(img.convert('L').resize((200, 2=00), Image.ANTIALIAS)))
                    if self.mode == 'RGB':
...
if __name__ == "__main__":
    P = PicMerge(pic_path='IMG_20190328_173607.jpg', mode='RGB')
    P.merge()